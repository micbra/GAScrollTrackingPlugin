...
scrollDepthTracker.prototype.onScroll = function(){
  ...
  var accuracy = self.config.accuracy;

  if(temp > this.reach){
    if(temp > 100){
      this.reach = 100
    } else {
      this.reach = Math.floor(temp / accuracy) * accuracy;
    }
  }
...
scrollDepthTracker.prototype.onUnload = function(){
  ...
  var data = {
    ...
    eventAction: (this.everScrolled) ? this.config.labelScroll : this.config.labelNoScroll,
    eventLabel: (this.everScrolled) ? this.reach.toString() : '',
    eventValue: 0,
    nonInteraction: true,
